...
const LEADERBOARD_CATEGORIES = [
  { label: "Lojas", key: "rank" as const, tab: "contributors" },
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const localProgress =
          parseInt(
            localStorage.getItem("gitcity_rabbit_progress") ?? "0",
            10,
          ) || 0;
    ...
    try {
      ...
      if (!devRes.ok) {
        ...
        if (devRes.status === 404) code = "not-found";
          ...
          code = devData.error?.includes("GitHub")
            ? "github-rate-limit"
            : "rate-limit";
    ...
    <main className="relative min-h-screen overflow-hidden bg-bg font-pixel uppercase text-warm">
                  ...
                  if (data) {
                    setShowFlyResults((prev) =>
                      prev
                        ? {
                            ...prev,
                            rank: data.rank_today ?? 0,
                            totalPilots: data.total ?? 0,
                          }
                        : null,
                    );
                  }
          ...
          <div className="absolute top-4 right-3 text-right text-[9px] text-muted sm:right-4 sm:text-[10px]">
            ...
            <div className="mt-1.5 text-[8px]">
              ...
              >
                ...
                {String(flyElapsedSec % 60).padStart(2, "0")}
              </span>
            </div>
            {flyPersonalBest > 0 && (
      ...
      {!flyMode && !introMode && !rabbitCinematic && (
        ...
        >
          ...
          >
            <img
              src="https://empreender.nyc3.digitaloceanspaces.com/landingpage/688cf519a8ea4.png"
              alt=""
              ...
              {MILESTONE_MODE === "stars"
                ? // ── GitHub Stars mode ──
                  (() => {
                    ...
                    const prev =
                      STAR_MILESTONES[STAR_MILESTONES.indexOf(target) - 1] ?? 0;
                    const progress =
                      ((starCount - prev) / (target - prev)) * 100;
                    ...
                    <button
                      onClick={() => {
                        ...
                        try {
                          setFlyPersonalBest(
                            parseInt(
                              localStorage.getItem("gitcity_fly_pb") || "0",
                              10,
                            ) || 0,
                          );
                        } catch {
                        ...
                        if (
                          !localStorage.getItem("gitcity_fly_controls_seen")
                        ) {
